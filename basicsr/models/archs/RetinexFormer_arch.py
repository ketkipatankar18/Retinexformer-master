...
class Illumination_Estimator(nn.Module):
    def __init__(
        ...
        # self, n_fea_middle, n_fea_in=3, n_fea_out=3):
...
class RetinexFormer_Single_Stage(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, level=2, num_blocks=[1, 1, 1]):
        print('In RetinexFormer_Single_Stage in_channels='+str(in_channels)+' n_feat= '+str(n_feat)+' level= '+str(level)+' num_blocks '+str(num_blocks))
...
# tar_path="/Applications/anaconda3/envs/dip/RETINEXFORMER/Retinex/Retinexformer-master/data/NTIRE/mini_val/target/1.png"
        ...
        # psnr_values.append(PSNR(target, restored))
        psnr_values.append(calculate_psnr(target, restored))
        ssim_values.append(calculate_ssim(
                        img_as_ubyte(target), img_as_ubyte(restored)))
        
        save_img((os.path.join("drive/MyDrive/Retinexformer-master/ModelOutput", os.path.splitext(
