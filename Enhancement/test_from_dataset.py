...
#                     help='Test Dataset') 
parser.add_argument(
    '--opt', type=str, default='../../Options/RetinexFormer_NTIRE.yml', help='Path to option YAML file.')
parser.add_argument('--weights', default='pretrained_weights/NTIRE.pth',
                    type=str, help='Path to weights')
parser.add_argument('--dataset', default='NTIRE', type=str,
                    help='Test Dataset') 
...
# opt = parse_options(args.opt, is_train=False)
print('After parse_options '+str(opt))
# opt = yaml_load(opt)
opt['dist'] = False
opt['num_gpu'] = 0
# opt= dict(opt)
...
print('displaying: '+str(create_model(opt).net_g))
model_restoration = create_model(opt).net_g
# model_restoration = build_model(opt).net_g
print('displying model restoration'+str(model_restoration))
# 加载模型
...
if dataset in ['SID', 'SMID', 'SDSD_indoor', 'SDSD_outdoor']:
    ...
    with torch.inference_mode():
        for inp_path, tar_path in tqdm(zip(input_paths, target_paths), total=len(target_paths)):
            ...
            # print('Inference step input=' +str(img))

            # print('Inference step target=' +str(img))
